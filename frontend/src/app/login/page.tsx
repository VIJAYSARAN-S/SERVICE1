    ...
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center p-4">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-border bg-white shadow-xl">
        <div className="bg-primary/5 p-8 text-center pb-6">
          ...
          <p className="mt-2 text-sm text-muted">Secure Access to Government Digital Services</p>
        </div>
        ...
        {showOtpToast && (
          <div className="fixed top-6 right-6 z-[100] animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="flex items-center gap-4 rounded-2xl border-2 border-primary bg-white p-5 shadow-2xl ring-4 ring-primary/10">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-primary">Simulation: OTP Received</p>
                <h4 className="text-xl font-black text-navy tracking-widest mt-1">{demoOtp}</h4>
                <p className="text-[10px] text-muted font-medium mt-1">Check your "registered" email device.</p>
              </div>
              <button 
                onClick={() => setShowOtpToast(false)}
                className="ml-4 text-muted hover:text-navy transition-colors"
              >
            ...
            <div className="mt-2 h-1 w-full bg-primary/20 rounded-full overflow-hidden">
               <div className="h-full bg-primary animate-out slide-out-to-left duration-[4000ms] ease-linear"></div>
            </div>
          </div>
        )}
        ...
        <div className="p-8 pt-6">
          ...
          {riskData && (
            <div className={`mb-6 rounded-lg p-4 text-sm font-medium ${
              ...
              Security Scan: {riskData.risk_status} (Score: {riskData.risk_score}/100)
              {riskData.risk_status !== 'BLOCKED' && <p className="mt-1 text-xs opacity-80">Finalizing secure session...</p>}
            </div>
          )}
